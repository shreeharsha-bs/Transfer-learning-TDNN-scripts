...
stage=16
nj=5
tag=hindi_model_2     #experiment tag, so that models are not overwritten; make sure change this if running a new experiment.
expdir=exp_${tag} 
datadir=data 
...
#local/nnet3/run_ivector_common_test_IITM.sh \  Use this for testing # some parameter errors might have to remove stage if parse options is done after
#  --stage $stage --nj $nj \
...
#local/nnet3/vad_run_ivector_common_IITM.sh \ # Use this for training
#  --stage $stage --nj $nj \
...
for f in $train_data_dir/feats.scp $train_ivector_dir/ivector.scp \
