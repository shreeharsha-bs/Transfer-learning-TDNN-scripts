...
adapt_no_sp="UP_RJ_new_Hindi_CS_train" #"UP_RJ_noise_aug" #"UP_RJ_Hindi_CS_train"
adapt=${adapt_no_sp}_sp #hindi_adapt_RJ
test_sets="UP_RJ_new_valid_all" # actually valid, don't change the name
...
common_egs_dir=
primary_lr_factor=0.01 # learning-rate factor for all except last layer in transferred source model, # no fine tuning other layers if this is 0
out_lr_factor=0.005 # learning rate factor for output layers
...
#for primary_lr_factor in 0.01;do
#for out_lr_factor in {0.0,0.005,1.0};do
echo $primary_lr_factor
echo $out_lr_factor
#cp -r exp_hindi_model_UP_noise_ready exp_hindi_model_1
...
#echo "Output layer frozen at 0.0"
  $train_cmd $dir/log/generate_input_mdl.log \
    nnet3-am-copy --raw=true --edits="set-learning-rate-factor name=* learning-rate-factor=$primary_lr_factor; set-learning-rate-factor name=output* learning-rate-factor=$out_lr_factor" \
...
#./decode_run.sh
mv exp_hindi_model_1 exp_hindi_model_UP_RJ_ILR_${primary_lr_factor}_out_${out_lr_factor}
#done
#done
exit 0
#if [ $stage -le 8 ]; then
