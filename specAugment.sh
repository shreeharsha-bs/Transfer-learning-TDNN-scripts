...
treedir=$exp_fold/chain/tree_new
...
if [ $stage -le 4 ]; then
  ...
  steps/nnet3/align_lats.sh --nj 5 --cmd "$train_cmd" $ivec_opt \
    ...
    $datadir/${adapt}_hires data/lang_generic_0.2/all-stories_0 $src_mdl_dir $lat_dir || exit 1;
  rm $lat_dir/fsts.*.gz # save space
fi
#exit 0

if [ $stage -le 5 ]; then
#lat_dir=/home/asr/kaldi/egs/kaldi_lets_asr/results/eng_train_dahanu_sp_valid_nsp_wav/ali/
#leaves=6000
   #Build a tree using our new topology.
  # steps/nnet3/chain/build_tree.sh --alignment_subsampling_factor 1 --frame-subsampling-factor 3 \
  #  --leftmost-questions-truncate -1 \
  #  --cmd "$train_cmd" $leaves $datadir/${adapt}_hires $src_lang $lat_dir $treedir || exit 1;

  echo "$0: Create neural net configs using the xconfig parser for";
  echo " adding spec aug layers, These layers ";
  echo " are at the top of the corresponding part of the IITM network. Check the dimensions and input for them.... harsha!!!";
  ...
  #spec-augment-layer name=spec-augment freq-max-proportion=0.5 time-zeroed-proportion=0.1 time-mask-max-frames=20 include-in-init=true
  ...
  ## the layers below are not used, it's just to satisfy xconfig script
  #relu-renorm-layer name=prefinal-chain input=prefinal-l l2-regularize=0.01 dim=1024
  #output-layer name=output include-log-softmax=false dim=$num_targets l2-regularize=0.005
  #relu-renorm-layer name=prefinal-xent input=prefinal-l l2-regularize=0.01 dim=1024
  #output-layer name=output-xent dim=$num_targets learning-rate-factor=5.0 l2-regularize=0.005
#EOF
  steps/nnet3/xconfig_to_configs.py --existing-model $src_mdl \
    --xconfig-file  $dir/configs/network.xconfig  \
    --config-dir $dir/configs/
#exit 0
# WSJ --> RM transfer example
#  relu-renorm-layer name=prefinal-chain input=tdnnf13.noop dim=450 target-rms=0.5
...
#	traine.frames-per-iter was 1000000
